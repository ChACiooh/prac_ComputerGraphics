...
def render():
    global gCamAng, gCamHeight, gAlpha, gBeta, gGamma, gKeyPressed
    ...
    Rz = np.array([[np.cos(zang), -np.sin(zang), 0],
                   ...
                   [0,0,1]])
    Rz_ = np.array([[np.cos(z_ang), -np.sin(z_ang), 0],
    ...
    if gKeyPressed != None:
        #print(R @ R.T)
        print('α:{} β:{} γ:{}'.format(gAlpha, gBeta, gGamma))
        print('Pressed Key:{}'.format(chr(gKeyPressed)))
        gKeyPressed = None
...
def key_callback(window, key, scancode, action, mods):
    global gCamAng, gCamHeight, gCamHeight, gAlpha, gBeta, gGamma, gKeyPressed
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'8-1', None,None)
    if not window:
        glfw.terminate()
        return

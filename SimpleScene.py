...
def getSplinePos(animTime):
    global cow_ctrl_pts

    idx = int(animTime)
    t = animTime - idx

    # Use Catmull-Rom Matrix for B-spline basis
    # requirement c)
    Catmull_Rom_Mtrx = np.array([[-.5, 1.5, -1.5, .5],
                                 [1., -2.5, 2., -.5],
                                 [-.5, 0., .5, 0.],
                                 [0., 1., 0., 0.]])
    """Hermite_Mtrx = np.array([[2., -2., 1., 1.],
                             [-3., 3., -2., -1],
                             [0., 0., 1., 0.],
                             [1., 0., 0., 0.]])"""
    
    int_time = int(animTime)
    dt = animTime - int_time
    act_pts = []

    # next control point to translate
    for i in range(4):
        t = int_time + i - 1
        lccp = len(cow_ctrl_pts)
        j = lccp - 1
        if t < 0:
            j = 0
        elif t < lccp:
            j = t
        act_pts.append(getTranslation(cow_ctrl_pts[j]))

    T = np.array([pow(dt, 3), pow(dt,2), dt, 1])
    P = np.vstack((act_pts[0], act_pts[1], act_pts[2], act_pts[3]))

    return T @ Catmull_Rom_Mtrx @ P

def actCow(animTime):
    global cow2wld 
    source = getSplinePos(animTime - .2)
    target = getSplinePos(animTime + .2)
    dir = target - source

    a = np.array([.0, .1, .0])
    u = normalize(np.cross(dir, a))   # normalize is defined in Ray.py
    v = normalize(np.cross(u, dir))
    w = np.cross(v, u)
    M_act_cow = np.eye(4)  # Initialize with identity matrix, basically float type
    ...
    #print('v :', v)
    #print('u :', u)
    #print('matrix :', M_act_cow)

    setTranslation(M_act_cow, getTranslation(cow2wld))
    cow2wld = M_act_cow

def display():
    global cameraIndex, cow2wld, is_picking, cow_ctrl_pts, isDrag
    ...
    if is_picking == False:
        animTime = glfw.get_time() - animStartTime
        spline = getSplinePos(animTime)
        isDrag = 0      # while act cow, disable the drag mode.
        actCow(animTime)
        setTranslation(cow2wld, spline)
        drawCow(cow2wld, cursorOnCowBoundingBox)

        if animTime >= len(cow_ctrl_pts):   # during end phase
            is_picking = True
            cow2wld = cow_ctrl_pts[0]
            cow_ctrl_pts.clear()
    else:
        drawCow(cow2wld, cursorOnCowBoundingBox)
        for i_cow in cow_ctrl_pts:
            drawCow(i_cow, None)
...
def set4TurnCow():
    global animStartTime, cow_ctrl_pts, NUM_TURN
    animStartTime = glfw.get_time()
    
    # print(cow2wld)
    tmp = []
    #print('ccp len :', len(cow_ctrl_pts))
    for i in range(NUM_TURN):
        for ccp in cow_ctrl_pts:
            tmp.append(ccp)

    tmp.append(cow_ctrl_pts[0])
    cow_ctrl_pts = tmp

def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, cow2wld, is_picking, cow_ctrl_pts
    GLFW_DOWN=1;
    GLFW_UP=0;
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            ...
            isDrag=H_DRAG;
            if is_picking == True:
                cow_ctrl_pts.append(cow2wld.copy())

                if len(cow_ctrl_pts) >= 6:
                    is_picking = False
                    set4TurnCow()
...
def onMouseDrag(window, x, y):
    ...
    if isDrag==V_DRAG or isDrag==H_DRAG: # modified to work for picking only.
        ...
        if  isDrag==V_DRAG:
            ...
            if cursorOnCowBoundingBox:
                ...
                T = np.eye(4)  # Identity matrix, basically float type
                now_y = ray.getPoint(c[1])[1] - getTranslation(cow2wld)[1]
                #print('now_y :', now_y)
                now_pos = np.array([0, now_y, 0])
                setTranslation(T, now_pos)
                cow2wld = T @ cow2wld

                now_x, now_z = ray.getPoint(c[1])[0], ray.getPoint(c[1])[2]
                print('vdrag at line via (x:{},z:{})'.format(int(now_x), int(now_z)))
            ...
            if cursorOnCowBoundingBox:
                ...
                print('hdrag at y={} plane'.format(int(ray.getPoint(c[1])[1])))
    else:
        ray=screenCoordToRay(window, x, y)
...
def main():
    ...
    width = 800;
    height = 600;
    window = glfw.create_window(width, height, 'PA2_ygwon_pyo', None, None)
    if not window:
        glfw.terminate()
        sys.exit(-1)
